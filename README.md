...
Generate a self-contained bash script equivalent of `bnn apply`. Useful for running on machines without bnn installed.

```bash
bnn export              # print to stdout
bnn export -o setup.sh  # write to file
```
...
# Generated by bnn export

# --- bunch: ruby ---
if mise current ruby | grep 3.3 2>/dev/null; then
  echo "[bnn] ruby: already configured, skipping"
else
...
- Bunches are written in dependency-resolved order
- `check` becomes an `if/else/fi` idempotency guard
- `runtime(mise, ...)` emits `mise install` + `mise global`
- `runtime(brew)` emits `brew install`
- `runtime(shell)` emits no runtime line
